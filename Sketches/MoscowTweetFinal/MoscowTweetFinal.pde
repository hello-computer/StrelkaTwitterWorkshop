...
// ПОЖАЛУЙСТА ПОЛУЧИТЕ СВОИ КЛЮЧИ (ИНСТРУКЦИЯ НИЖЕ)
// ЕСЛИ ДВА ЧЕЛОВЕКА ЗАПУСТЯТ ПРИЛОЖЕНИЕ С ОДИНАКОВЫМИ КЛЮЧАМИ ОДНОВРЕМЕННО ТВИТТЕР ВРЕМЕННО ЗАБЛОКИРУЕТ ДОСТУП
...
void keyPressed() {
    // Дейсвтия для клавиши S
    if (key == 's' || key == 'S') {  
        
        // Задаем имя будущего файла
        String name = "moscowtweet-";
        // Добавляем к нему дату
        ...
        // Выводим сообщение в консоль о том что изоражение сохранено
        println("Image saved with name" + name);
    }
}
